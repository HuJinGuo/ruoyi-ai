...
import XtpEntityDrawer from './xtp-entity-drawer.vue';

const props = defineProps<{
  config: XtpPageConfig;
}>();

const lookups = ref<XtpLookups>(getEmptyLookups());
...
const gridOptions: VxeGridProps = {
  ...
  id: `xtp-${props.config.rowKey}-index`,
};
...
const [EntityDrawer, drawerApi] = useVbenDrawer({
  connectedComponent: XtpEntityDrawer,
});

onMounted(async () => {
  lookups.value = await loadXtpLookups();
...
function handleEdit(record: XtpEntity) {
  drawerApi.setData({
    config: props.config,
    id: getId(record),
...
async function handleDelete(row: XtpEntity) {
